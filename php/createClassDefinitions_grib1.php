...
function process_template($file, $name, $class, $outdir)
{
  ...
  foreach ( $allRows as $row )
  {
    ...
    if (preg_match ( "/\s*include/", $row ))
    {
      ...
      if (isset ( $incclass ))
      {
        ...
        $headers ["\"grib1/definition/${incclass}.h\""] = 1;
  ...
  $cpp .= "// ***********************************************************************\n\n"; 
  $cpp .= "#include \"grib1/definition/${class}.h\"\n";
  ...
  $cpp .= "${class}::~${class}() { try { } catch (...) {throw SmartMet::Spine::Exception(BCP,\"Operation failed!\",NULL);}\n}\n\n";
