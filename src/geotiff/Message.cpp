...
Message::Message()
{
  ...
  try
  {
    mGridFile = nullptr;
    mMessageIndex = 0;
    mFileType = T::FileTypeValue::GeoTiff;
    mParameterLevel = 0;
    mForecastType = 0;
    mForecastNumber = 0;
    mForecastTimeT = 0;
...
:GRID::Message(message)
{
  ...
  try
  {
    ...
    mGeoTiffFile = message.mGeoTiffFile;
    mGeometryDef = message.mGeometryDef;
    mDataPtr = message.mDataPtr;
    mIsRead = message.mIsRead;
    mDataSize = message.mDataSize;
    mDataType = message.mDataType;
    mBitsPerValue = message.mBitsPerValue;
  }
  catch (...)
  {
    throw Fmi::Exception(BCP,"Operation failed!",nullptr);
...
Message::Message(GRID::GridFile *gridFile,GeoTiffFile *geoTiffFile,T::MessageIndex messageIndex,GeoTiff::MessageInfo& messageInfo)
{
  ...
  {
    mGridFile = gridFile;
    mGeoTiffFile = geoTiffFile;
    mMessageIndex = messageIndex;
    mFileType = T::FileTypeValue::GeoTiff;
    mColumnCount = messageInfo.mColumns;
    mRowCount = messageInfo.mRows;

    mDataPtr = messageInfo.mDataPtr;
    mDataSize = messageInfo.mDataSize;
    mDataType = messageInfo.mDataType;
    mBitsPerValue = messageInfo.mBitsPerValue;

    mFmiParameterLevelId = messageInfo.mLevelId;
    mParameterLevel = messageInfo.mLevel;
    mForecastType = 1;
    mForecastNumber = 0;
    ...
    Identification::gridDef.getFmiParameterDefByName(messageInfo.mParameterName,fmiDef);
...
void Message::getGridLatLonCoordinatesByGridPointList(std::vector<T::Point>& gridPoints,T::Coordinate_vec& coordinates,std::vector<bool>& found) const
{
  ...
  try
  {
    if (!mGeometryDef)
    {
      coordinates.assign(gridPoints.size(),T::Coordinate(0,0));
      found.assign(gridPoints.size(),false);
      return;
    }

    mGeometryDef->getGridLatLonCoordinatesByGridPointList(gridPoints,coordinates,found);
  }
  catch (...)
  {
    Fmi::Exception exception(BCP,"Operation failed!",nullptr);
...
T::ParamValue Message::getGridValueByGridPoint(uint grid_i,uint grid_j) const
{
  ...
  try
  {
    if (!mDataPtr || grid_i >= mColumnCount || grid_j >= mRowCount)
      return ParamValueMissing;

    uint idx = grid_j * mColumnCount + grid_i;
    switch (mDataType)
    {
      case 1: // unsigned integer
        if (mBitsPerValue == 32)
        {
          uint *data = (uint*)mDataPtr;
          return (T::ParamValue)data[idx];
        }
        if (mBitsPerValue == 16)
        {
          ushort *data = (ushort*)mDataPtr;
          return (T::ParamValue)data[idx];
        }
        if (mBitsPerValue == 8)
        {
          uchar *data = (uchar*)mDataPtr;
          return (T::ParamValue)data[idx];
        }
        break;
      ...
      case 2: // signed integer
        if (mBitsPerValue == 32)
        {
          int *data = (int*)mDataPtr;
          return (T::ParamValue)data[idx];
        }
        if (mBitsPerValue == 16)
        {
          short *data = (short*)mDataPtr;
          return (T::ParamValue)data[idx];
        }
        if (mBitsPerValue == 8)
        {
          char *data = (char*)mDataPtr;
          return (T::ParamValue)data[idx];
        }
        break;
...
void Message::getGridMinAndMaxValues(T::ParamValue& minValue,T::ParamValue& maxValue) const
{
  try
  {
    ...
    for (uint y=0; y<mRowCount; y++)
    {
      for (uint x=0; x<mColumnCount; x++)
      {
...
void Message::print(std::ostream& stream,uint level,uint optionFlags) const
{
  ...
  try
  {
    ...
    stream << space(level) << "- dataSize               = " << toString(mDataSize) << " (" << uint64_toHex(mDataSize) << ")\n";
    stream << space(level) << "- columns                = " << toString(mColumnCount) << "\n";
    stream << space(level) << "- rows                   = " << toString(mRowCount) << "\n";
    stream << space(level) << "- dataType               = " << toString(mDataType) << "\n";
    stream << space(level) << "- bitsPerValue            = " << std::to_string(mBitsPerValue) << "\n";
