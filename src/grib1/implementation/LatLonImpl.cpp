...
bool LatLonImpl::getGridPointByOriginalCoordinates(double x,double y,double& grid_i,double& grid_j) const
{
  try
  {
    ...
    if (i < 0 && (int)(i*100000) == 0)
      i = 0;

    if (i >= C_DOUBLE(mNi) && (int)((i-mNi)*100000) == 0)
      i = C_DOUBLE(mNi)-0.000001;

    if (j < 0 && (int)(j*100000) == 0)
      j = 0;

    if (j >= C_DOUBLE(mNj) && (int)((i-mNj)*100000) == 0)
      j = C_DOUBLE(mNj)-0.000001;
...
bool LatLonImpl::getProperty(uint propertyId,Int64& value)
{
  try
  {
    switch (propertyId)
...
bool LatLonImpl::setProperty(uint propertyId,Int64 value)
{
  try
  {
    switch (propertyId)
