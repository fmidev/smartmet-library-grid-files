...
void ComplexSphericalHarmonicsDataRepresentation::read(
...
void ComplexSphericalHarmonicsDataRepresentation::getAttributeList(
    std::string prefix, T::AttributeList &attributeList) const {
  try {
    char name[300];
    sprintf(name, "%sComplexSphericalHarmonicsDataRepresentation.",
            prefix.c_str());
    mPacking.getAttributeList(name, attributeList);
    sprintf(
        name,
        "%sComplexSphericalHarmonicsDataRepresentation.LaplacianScalingFactor",
        prefix.c_str());
    attributeList.addAttribute(name, toString(mLaplacianScalingFactor));
    sprintf(name, "%sComplexSphericalHarmonicsDataRepresentation.TS",
            prefix.c_str());
    attributeList.addAttribute(name, toString(mTS));
    sprintf(
        name,
        "%sComplexSphericalHarmonicsDataRepresentation.UnpackedSubsetPrecision",
        prefix.c_str());
...
void ComplexSphericalHarmonicsDataRepresentation::print(
    std::ostream &stream, uint level, uint optionFlags) const {
  try {
    stream << space(level) << "ComplexSphericalHarmonicsDataRepresentation\n";
    mPacking.print(stream, level + 1, optionFlags);
    stream << space(level)
           << "- LaplacianScalingFactor = " << toString(mLaplacianScalingFactor)
           << "\n";
    stream << space(level) << "- TS = " << toString(mTS) << "\n";
    stream << space(level) << "- UnpackedSubsetPrecision = "
           << toString(mUnpackedSubsetPrecision) << "\n";
  } catch (...) {
    throw SmartMet::Spine::Exception(BCP, "Operation failed!", NULL);
  }
}
...
ComplexSphericalHarmonicsDataRepresentation::getPacking() const {
  try {
    return &mPacking;
  } catch (...) {
    throw SmartMet::Spine::Exception(BCP, "Operation failed!", NULL);
...
/*! \brief The method returns the value of the {@link mLaplacianScalingFactor}
 * attribute. */

const T::Int32_opt &
ComplexSphericalHarmonicsDataRepresentation::getLaplacianScalingFactor() const {
  try {
...
const T::UInt32_opt &
ComplexSphericalHarmonicsDataRepresentation::getTS() const {
  try {
    return mTS;
  } catch (...) {
    throw SmartMet::Spine::Exception(BCP, "Operation failed!", NULL);
...
/*! \brief The method returns the value of the {@link mUnpackedSubsetPrecision}
 * attribute. */

const T::UInt8_opt &
ComplexSphericalHarmonicsDataRepresentation::getUnpackedSubsetPrecision()
    const {
  try {
