...
void DeprecatedSatelliteProduct::write(DataWriter &dataWriter) {
  try {
...
bool DeprecatedSatelliteProduct::getAttributeValue(const char *attributeName, std::string &attributeValue) const {
  try {
...
bool DeprecatedSatelliteProduct::hasAttributeValue(const char *attributeName, const char *attributeValue) const {
  try {
...
T::Hash DeprecatedSatelliteProduct::countHash() const {
  try {
    std::size_t seed = 0;
...
uint DeprecatedSatelliteProduct::getTemplateNumber() const {
  return 30;
}

ProductDefinition *DeprecatedSatelliteProduct::createProductDefinition() const {
  try {
    return static_cast<ProductDefinition *>(new DeprecatedSatelliteProduct(*this));
  } catch (...) {
    throw Fmi::Exception(BCP, "Operation failed", nullptr);
  }
}
