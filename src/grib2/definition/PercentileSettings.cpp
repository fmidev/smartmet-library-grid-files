...
bool PercentileSettings::getAttributeValue(const char *attributeName, std::string &attributeValue) const {
  try {
    if (attributeName == nullptr)
      return false;
    if (strcasecmp(attributeName, "PercentileValue") == 0) {
      attributeValue = toString(mPercentileValue);
      return true;
    }
    return false;
  } catch (...) {
...
bool PercentileSettings::hasAttributeValue(const char *attributeName, const char *attributeValue) const {
  try {
    if (attributeName == nullptr || attributeValue == nullptr)
      return false;
    if (strcasecmp(attributeName, "PercentileValue") == 0 && strcasecmp(attributeValue, toString(mPercentileValue).c_str()) == 0)
      return true;
    return false;
  } catch (...) {
    throw Fmi::Exception(BCP, "Operation failed", nullptr);
...
T::Hash PercentileSettings::countHash() const {
  try {
    std::size_t seed = 0;
    if (mPercentileValue)
      Fmi::hash_merge(seed, *mPercentileValue);
    return seed;
  } catch (...) {
    throw Fmi::Exception(BCP, "Operation failed", nullptr);
  }
