...
bool PngGridDataRepresentation::getAttributeValue(const char *attributeName, std::string &attributeValue) const {
  try {
...
bool PngGridDataRepresentation::hasAttributeValue(const char *attributeName, const char *attributeValue) const {
  try {
...
T::Hash PngGridDataRepresentation::countHash() const {
  try {
    std::size_t seed = 0;
    return seed;
  } catch (...) {
