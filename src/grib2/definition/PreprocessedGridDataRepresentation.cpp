...
//   you should write them into the 'PreprocessedGridDataRepresentationImpl.*'
//   files.
// ***********************************************************************
...
void PreprocessedGridDataRepresentation::getAttributeList(
  ...
  try {
    ...
    sprintf(name, "%sPreprocessedGridDataRepresentation.TypeOfPreProcessing",
            prefix.c_str());
    attributeList.addAttribute(name, toString(mTypeOfPreProcessing));
    sprintf(name, "%sPreprocessedGridDataRepresentation.PreProcessingParameter",
            prefix.c_str());
...
void PreprocessedGridDataRepresentation::print(std::ostream &stream, uint level,
                                               uint optionFlags) const {
  try {
    stream << space(level) << "PreprocessedGridDataRepresentation\n";
    mPacking.print(stream, level + 1, optionFlags);
    stream << space(level)
           << "- TypeOfPreProcessing = " << toString(mTypeOfPreProcessing)
           << "\n";
    stream << space(level)
           << "- PreProcessingParameter = " << toString(mPreProcessingParameter)
           << "\n";
  } catch (...) {
    throw SmartMet::Spine::Exception(BCP, "Operation failed!", NULL);
  }
...
/*! \brief The method returns the value of the {@link mTypeOfPreProcessing}
 * attribute. */

const T::UInt8_opt &
PreprocessedGridDataRepresentation::getTypeOfPreProcessing() const {
  try {
...
/*! \brief The method returns the value of the {@link mPreProcessingParameter}
 * attribute. */
