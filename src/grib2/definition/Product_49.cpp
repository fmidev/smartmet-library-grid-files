...
void Product_49::getAttributeList(const std::string &prefix, T::AttributeList &attributeList) const {
  try {
    char name[300];
...
bool Product_49::getAttributeValue(const char *attributeName, std::string &attributeValue) const {
  try {
    if (attributeName == nullptr)
      return false;
    if (mParameterAerosolOptical.getAttributeValue(attributeName, attributeValue))
      return true;
...
bool Product_49::hasAttributeValue(const char *attributeName, const char *attributeValue) const {
  try {
    if (attributeName == nullptr || attributeValue == nullptr)
      return false;
    if (mParameterAerosolOptical.hasAttributeValue(attributeName, attributeValue))
      return true;
...
T::Hash Product_49::countHash() const {
  try {
    std::size_t seed = 0;
    return seed;
  } catch (...) {
