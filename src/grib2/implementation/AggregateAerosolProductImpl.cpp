...
const T::UInt8_opt AggregateAerosolProductImpl::getGribParameterCategory() const
{
  try
  {
    return mParameterAerosol.getParameterCategory();
  }
  catch (...)
  {
    throw Fmi::Exception(BCP, "Operation failed!", nullptr);
...
const T::UInt8_opt AggregateAerosolProductImpl::getGribParameterNumber() const
{
  try
  {
    return mParameterAerosol.getParameterNumber();
  }
  catch (...)
  {
    throw Fmi::Exception(BCP, "Operation failed!", nullptr);
...
T::UInt8_opt AggregateAerosolProductImpl::getGeneratingProcessIdentifier() const
{
  try
  {
    return mParameterAerosol.getGeneratingProcessIdentifier();
  }
  catch (...)
  {
    throw Fmi::Exception(BCP, "Operation failed!", nullptr);
...
T::TimeString AggregateAerosolProductImpl::countForecastStartTime(T::TimeString referenceTime) const
{
  try
  {
    ...
    auto forecastTimeP = mParameterAerosol.getForecastTime();
    if (!forecastTimeP)
    {
      throw Fmi::Exception(BCP, "The 'mParameterChemical.forecastTime' value not defined!");
    }

    auto indicator = mParameterAerosol.getIndicatorOfUnitOfTimeRange();
    if (!indicator)
