...
ComplexGridDataRepresentationImpl::ComplexGridDataRepresentationImpl()
{
}
...
ComplexGridDataRepresentationImpl::ComplexGridDataRepresentationImpl(const ComplexGridDataRepresentationImpl& other)
:ComplexGridDataRepresentation(other)
{
}
...
RepresentationDefinition* ComplexGridDataRepresentationImpl::createRepresentationDefinition() const
{
  try
  {
    return (RepresentationDefinition*) new ComplexGridDataRepresentationImpl(*this);
  }
  catch (...)
  {
...
void ComplexGridDataRepresentationImpl::read(MemoryReader& memoryReader)
{
  try
  {
    ComplexGridDataRepresentation::read(memoryReader);
  }
  catch (...)
  {
    throw Fmi::Exception(BCP,"Operation failed!",nullptr);
...
/*
   ...
   // ### use "ParamValueMissing".
    ...
    float missingValue1 = 0;
    float missingValue2 = 0;

    if (*mMissingValueManagementUsed == 1)
      missingValue1 = (float)*mPrimaryMissingValueSubstitute;

    if (*mMissingValueManagementUsed == 2)
    {
      missingValue1 = (float)*mPrimaryMissingValueSubstitute;
      missingValue2 = (float)*mSecondaryMissingValueSubstitute;
    }
*/
        ...
        if (groupValueSize[g] != 0)
        {
          ...
          for (int k=0; k<groupSize[g]; k++)
          {
            ...
            else
            {
               ...
               dataValue[c++] = dataValue[n] + groupReference[g];
            }
            n++;
          }
          readPosition = readPosition + (groupValueSize[g] * groupSize[g]);
