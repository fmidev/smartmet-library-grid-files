...
void LambertConformalImpl::getGridLatlonAreaCoordinates(double& firstLat,double& firstLon,double& lastLat,double& lastLon) const
{
  try
  {
    ...
    firstLat = latitudeOfFirstGridPoint;
    firstLon = longitudeOfFirstGridPoint;

    if (firstLon > 180)
      firstLon = firstLon - 360;
    ...
    double yy = latitudeOfFirstGridPoint + ny*dy;
    double xx = longitudeOfFirstGridPoint + nx*dx;


    mCt_lambert2latlon->Transform(1,&xx,&yy);
