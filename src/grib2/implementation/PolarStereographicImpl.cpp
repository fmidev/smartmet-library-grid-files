...
bool PolarStereographicImpl::setProperty(uint propertyId,Int64 value)
{
  try
  {
    switch (propertyId)
...
void PolarStereographicImpl::init() const
{
  try
  {
    ...
    double lon = getLongitude(C_DOUBLE(*mLongitudeOfFirstGridPoint) / 1000000.0);
    double lat = C_DOUBLE(*mLatitudeOfFirstGridPoint) / 1000000.0;
