...
class ValueCache
{
  public:
    ...
    void      init(uint size,uint removeLimit);
  ...
  protected:
    ...
    /*! \brief An array of the access times . */
    time_t*   mAccessTimeList;
  ...
  public:
    ...
    inline bool getValue(uint key,uint index,T::ParamValue& value)
    {
      try
      {
        ...
        {
          // There is no value vector available with this key or the value vector is cached
          // with a diffrent key.
