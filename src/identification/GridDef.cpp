...
make_ptr(OGRCoordinateTransformation *transformation) {
  const auto destroy_transformation =
      [](OGRCoordinateTransformation *transformation) {
        OCTDestroyCoordinateTransformation(transformation);
      };
  return std::shared_ptr<OGRCoordinateTransformation>(transformation,
...
void GridDef::init(const char *configFile) {
  ...
  try {
    const char *configAttribute[] = {
        ...
        "smartmet.library.grid-files.newbase.parameterDef[]", nullptr};
    ...
    mConfigurationFile.getAttributeValue(
        "smartmet.library.grid-files.grib.parameterDef",
        mGrib_parameterDef_files);
    mConfigurationFile.getAttributeValue(
        "smartmet.library.grid-files.grib1.parameterDef",
        mGrib1_parameterDef_files);
    mConfigurationFile.getAttributeValue(
        "smartmet.library.grid-files.grib2.parameterDef",
        mGrib2_parameterDef_files);
    mConfigurationFile.getAttributeValue(
        "smartmet.library.grid-files.fmi.parameterDef",
        mFmi_parameterDef_files);
    mConfigurationFile.getAttributeValue(
        "smartmet.library.grid-files.fmi.levelDef", mFmi_levelDef_files);
    mConfigurationFile.getAttributeValue(
        "smartmet.library.grid-files.fmi.forecastTypeDef",
        mFmi_forecastTypeDef_files);
    mConfigurationFile.getAttributeValue(
        "smartmet.library.grid-files.fmi.geometryDef", mFmi_geometryDef_files);
    mConfigurationFile.getAttributeValue(
        "smartmet.library.grid-files.fmi.parametersFromGrib",
        mFmi_parametersFromGrib_files);
    mConfigurationFile.getAttributeValue(
        "smartmet.library.grid-files.fmi.parametersFromNetCdf",
        mFmi_parametersFromNetCdf_files);
    mConfigurationFile.getAttributeValue(
        "smartmet.library.grid-files.fmi.parametersFromNewbase",
        mFmi_parametersFromNewbase_files);
    mConfigurationFile.getAttributeValue(
        "smartmet.library.grid-files.fmi.levelsFromGrib1",
        mFmi_levelsFromGrib1_files);
    mConfigurationFile.getAttributeValue(
        "smartmet.library.grid-files.fmi.levelsFromGrib2",
        mFmi_levelsFromGrib2_files);
    mConfigurationFile.getAttributeValue(
        "smartmet.library.grid-files.fmi.levelsFromNewbase",
        mFmi_levelsFromNewbase_files);
    mConfigurationFile.getAttributeValue(
        "smartmet.library.grid-files.netcdf.parameterDef",
        mNetCdf_parameterDef_files);
    mConfigurationFile.getAttributeValue(
        "smartmet.library.grid-files.newbase.parameterDef",
        mNewbase_parameterDef_files);
...
void GridDef::updateCheck() {
  ...
  try {
    if (!mInitialized)
      throw Fmi::Exception(
          BCP, "The 'GridDef' object is not initialized! Call init() first!");
...
void GridDef::updateGrib() {
  ...
  try {
    ...
    if (tt != mGrib_parameterDef_modificationTime) {
      ...
      for (auto it = mGrib_parameterDef_files.begin();
           it != mGrib_parameterDef_files.end(); ++it) {
...
void GridDef::updateGrib1() {
  ...
  try {
    ...
    if (tt != mGrib1_parameterDef_modificationTime) {
      ...
      for (auto it = mGrib1_parameterDef_files.begin();
           it != mGrib1_parameterDef_files.end(); ++it) {
...
void GridDef::updateGrib2() {
  ...
  try {
    ...
    if (tt != mGrib2_parameterDef_modificationTime) {
      ...
      for (auto it = mGrib2_parameterDef_files.begin();
           it != mGrib2_parameterDef_files.end(); ++it) {
...
void GridDef::updateFmi() {
  ...
  try {
    ...
    if (tt != mFmi_parameterDef_modificationTime) {
      ...
      mFmiNameToId.clear();
      for (auto it = mFmi_parameterDef_files.begin();
           it != mFmi_parameterDef_files.end(); ++it) {
    ...
    if (tt != mFmi_levelDef_modificationTime) {
      ...
      for (auto it = mFmi_levelDef_files.begin();
           it != mFmi_levelDef_files.end(); ++it) {
    ...
    if (tt != mFmi_forecastTypeDef_modificationTime) {
      ...
      for (auto it = mFmi_forecastTypeDef_files.begin();
           it != mFmi_forecastTypeDef_files.end(); ++it) {
    ...
    if (tt != mFmi_parametersFromGrib_modificationTime) {
      ...
      for (auto it = mFmi_parametersFromGrib_files.begin();
           it != mFmi_parametersFromGrib_files.end(); ++it) {
    ...
    if (tt != mFmi_parametersFromNewbase_modificationTime) {
      ...
      for (auto it = mFmi_parametersFromNewbase_files.begin();
           it != mFmi_parametersFromNewbase_files.end(); ++it) {
    ...
    if (tt != mFmi_parametersFromNetCdf_modificationTime) {
      ...
      for (auto it = mFmi_parametersFromNetCdf_files.begin();
           it != mFmi_parametersFromNetCdf_files.end(); ++it) {
    ...
    if (tt != mFmi_levelsFromGrib1_modificationTime) {
      ...
      for (auto it = mFmi_levelsFromGrib1_files.begin();
           it != mFmi_levelsFromGrib1_files.end(); ++it) {
    ...
    if (tt != mFmi_levelsFromGrib2_modificationTime) {
      ...
      for (auto it = mFmi_levelsFromGrib2_files.begin();
           it != mFmi_levelsFromGrib2_files.end(); ++it) {
    ...
    if (tt != mFmi_levelsFromNewbase_modificationTime) {
      ...
      for (auto it = mFmi_levelsFromNewbase_files.begin();
           it != mFmi_levelsFromNewbase_files.end(); ++it) {
    ...
    if (tt != mFmi_geometryDef_modificationTime) {
      /*
      for (auto it = mGridDefinitions1.begin(); it != mGridDefinitions1.end();
      ++it) delete (it->second);

      for (auto it = mGridDefinitions2.begin(); it != mGridDefinitions2.end();
      ++it) delete (it->second);
      */
      mGridDefinitions1.clear();
      mGridDefinitions2.clear();

      for (auto it = mFmi_geometryDef_files.begin();
           it != mFmi_geometryDef_files.end(); ++it) {
...
void GridDef::updateNewbase() {
  ...
  try {
    ...
    if (tt != mNewbase_parameterDef_modificationTime) {
      ...
      for (auto it = mNewbase_parameterDef_files.begin();
           it != mNewbase_parameterDef_files.end(); ++it) {
...
void GridDef::updateNetCdf() {
  ...
  try {
    ...
    if (tt != mNetCdf_parameterDef_modificationTime) {
      ...
      for (auto it = mNetCdf_parameterDef_files.begin();
           it != mNetCdf_parameterDef_files.end(); ++it) {
...
bool GridDef::getGrib1ParameterDefById(T::GribParamId gribParamId,
                                       Grib1ParameterDef &paramDef) {
  FUNCTION_TRACE
  try {
    updateCheck();
    AutoReadLock lock(&mModificationLock);
...
bool GridDef::getGrib1ParameterDefByFmiId(T::FmiParamId fmiParamId,
                                          Grib1ParameterDef &paramDef) {
  FUNCTION_TRACE
  try {
    updateCheck();
...
bool GridDef::getGrib1ParameterDefByIndex(uint index,
                                          Grib1ParameterDef &paramDef) {
  FUNCTION_TRACE
  try {
    updateCheck();
...
Grib1ParamDef_cptr
GridDef::getGrib1ParameterDefById(T::GribParamId gribParamId) {
  FUNCTION_TRACE
  try {
    for (auto it = mGrib1_parameterDef_records.begin();
         it != mGrib1_parameterDef_records.end(); ++it) {
...
Grib1ParamDef_cptr
GridDef::getGrib1ParameterDefByFmiId(T::FmiParamId fmiParamId) {
  FUNCTION_TRACE
  try {
...
bool GridDef::getGrib2ParameterDefById(T::GribParamId gribParamId,
                                       Grib2ParameterDef &paramDef) {
  FUNCTION_TRACE
  try {
    updateCheck();
    AutoReadLock lock(&mModificationLock);
...
bool GridDef::getGrib2ParameterDefByFmiId(T::FmiParamId fmiParamId,
                                          Grib2ParameterDef &paramDef) {
  FUNCTION_TRACE
  try {
    updateCheck();
...
bool GridDef::getGribParameterMappingByFmiId(T::FmiParamId fmiParamId,
                                             FmiParameterId_grib &def) {
  FUNCTION_TRACE
  try {
    updateCheck();
    AutoReadLock lock(&mModificationLock);
...
bool GridDef::getGrib2ParameterDefByIndex(uint index,
                                          Grib2ParameterDef &paramDef) {
  FUNCTION_TRACE
  try {
    updateCheck();
...
Grib2ParamDef_cptr
GridDef::getGrib2ParameterDefById(T::GribParamId gribParamId) {
  FUNCTION_TRACE
  try {
    for (auto it = mGrib2_parameterDef_records.begin();
         it != mGrib2_parameterDef_records.end(); ++it) {
      if (it->mGribParameterId == gribParamId)
        return &(*it);
    }
    return nullptr;
...
Grib2ParamDef_cptr
GridDef::getGrib2ParameterDefByFmiId(T::FmiParamId fmiParamId) {
  FUNCTION_TRACE
  try {
    FmiParamId_grib_cptr p = getGribParameterMappingByFmiId(fmiParamId);
    if (p)
...
bool GridDef::getGribParameterDefById(T::GribParamId gribParamId,
                                      GribParameterDef &paramDef) {
  FUNCTION_TRACE
  try {
    updateCheck();
    AutoReadLock lock(&mModificationLock);
...
bool GridDef::getGribParameterDefByName(const std::string &gribParamName,
                                        GribParameterDef &paramDef) {
  FUNCTION_TRACE
  try {
    updateCheck();
...
GribParamDef_cptr GridDef::getGribParameterDefById(T::GribParamId gribParamId) {
  FUNCTION_TRACE
  try {
    for (auto it = mGrib_parameterDef_records.begin();
         it != mGrib_parameterDef_records.end(); ++it) {
      if (it->mGribParameterId == gribParamId)
        return &(*it);
    }
    return nullptr;
...
GribParamDef_cptr
GridDef::getGribParameterDefByName(const std::string &gribParamName) {
  FUNCTION_TRACE
  try {
    for (auto it = mGrib_parameterDef_records.begin();
         it != mGrib_parameterDef_records.end(); ++it) {
...
bool GridDef::getFmiForecastTypeDef(int forecastTypeId,
...
LevelDef_cptr GridDef::getFmiLevelDef(uint levelId) {
  FUNCTION_TRACE
  try {
    for (auto it = mFmi_levelDef_records.begin();
         it != mFmi_levelDef_records.end(); ++it) {
...
ForecastTypeDef_cptr GridDef::getFmiForecastTypeDef(int forecastTypeId) {
  FUNCTION_TRACE
  try {
    for (auto it = mFmi_forecastTypeDef_records.begin();
         it != mFmi_forecastTypeDef_records.end(); ++it) {
        ...
        if (c > 1) {
          ...
          if (field[1][0] != '\0') {
            ...
            for (auto it = partList.begin(); it != partList.end(); ++it) {
              ...
              splitString(*it, '=', list);
              if (list.size() == 2) {
                std::pair<uint, std::string> rr(
                    stringFactory.create(list[0].c_str()), list[1]);
    ...
    while (!feof(file)) {
      if (fgets(st, 1000, file) != nullptr && st[0] != '#') {
        ...
        if (c > 2) {
          ...
          mFmi_parametersFromGrib_records.insert(
              std::pair<uint, FmiParameterId_grib>(rec.mFmiParameterId, rec));
          mGribIdToFmiId.insert(
              std::pair<uint, uint>(rec.mGribParameterId, rec.mFmiParameterId));
        }
      }
    }
    fclose(file);
    ...
    while (!feof(file)) {
      if (fgets(st, 1000, file) != nullptr && st[0] != '#') {
        ...
        if (c > 2) {
          ...
          mFmi_parametersFromNewbase_records.insert(
              std::pair<uint, FmiParameterId_newbase>(rec.mFmiParameterId,
                                                      rec));
          if (rec.mConversionFunction.empty())
            mNewbaseIdToFmiId.insert(std::pair<uint, uint>(
                rec.mNewbaseParameterId, rec.mFmiParameterId));

          if (rec.mReverseConversionFunction.empty())
            mFmiIdToNewbaseId.insert(std::pair<uint, uint>(
                rec.mFmiParameterId, rec.mNewbaseParameterId));
        }
      }
    }
    fclose(file);
    ...
    while (!feof(file)) {
      if (fgets(st, 1000, file) != nullptr && st[0] != '#') {
        ...
        if (c > 2) {
          ...
          mFmi_parametersFromNetCdf_records.insert(
              std::pair<uint, FmiParameterId_netCdf>(rec.mFmiParameterId, rec));
          std::string key = toUpperString(rec.mNetCdfParameterName);
          mNetCdfNameToFmiId.insert(
              std::pair<std::string, uint>(key, rec.mFmiParameterId));
        }
      }
    }
    fclose(file);
    ...
    while (!feof(file)) {
      if (fgets(st, 1000, file) != nullptr && st[0] != '#') {
        ...
        if (c > 3) {
          ...
          mFmi_parameterDef_records.insert(
              std::pair<uint, FmiParameterDef>(rec.mFmiParameterId, rec));
      ...
      if (fgets(st, 1000, file) != nullptr && st[0] != '#') {
        ...
        if (c >= 2) {
          ...
          if (newbaseLevelId != 0 || fmiLevelId != 0)
            mNewbaseLevelIdToFmiLevelId.insert(
                std::pair<uint, uint>(newbaseLevelId, fmiLevelId));
        }
      }
    }
    fclose(file);
    ...
    while (!feof(file)) {
      if (fgets(st, 1000, file) != nullptr && st[0] != '#') {
        ...
        if (c > 1) {
          ...
          mNewbase_parameterDef_records.insert(
              std::pair<uint, NewbaseParameterDef>(rec.mNewbaseParameterId,
                                                   rec));
          std::string key = toUpperString(rec.mParameterName);
          mNewbaseNameToId.insert(
              std::pair<std::string, uint>(key, rec.mNewbaseParameterId));
        }
      }
    }
    fclose(file);
    ...
    while (!feof(file)) {
      if (fgets(st, 1000, file) != nullptr && st[0] != '#') {
        ...
        if (c > 1) {
          ...
          mNetCdf_parameterDef_records.insert(
              std::pair<std::string, NetCdfParameterDef>(
                  toUpperString(rec.mParameterName), rec));
        }
      }
    }
    fclose(file);
...
bool GridDef::getGridLatLonAreaByGeometryId(T::GeometryId geometryId,
...
bool GridDef::getGridCellAverageSizeByGeometryId(T::GeometryId geometryId,
                                                 double &width,
                                                 double &height) {
  FUNCTION_TRACE
  try {
...
T::Coordinate_svec
GridDef::getGridOriginalCoordinatesByGeometryId(T::GeometryId geometryId) {
  FUNCTION_TRACE
  try {
    auto g2 = GridDef::getGrib2DefinitionByGeometryId(geometryId);
    if (g2)
...
T::Coordinate_svec
GridDef::getGridLatLonCoordinatesByGeometryId(T::GeometryId geometryId) {
  FUNCTION_TRACE
  try {
    auto g2 = GridDef::getGrib2DefinitionByGeometryId(geometryId);
    if (g2) {
...
void GridDef::getGridOriginalCoordinatesByGeometry(
    T::AttributeList &attributeList, T::Coordinate_svec &latLonCoordinates,
    T::CoordinateType coordinateType, T::Coordinate_svec &coordinates,
  ...
  try {
    ...
    const char *originalCrsStr =
        attributeList.getAttributeValue("grid.original.crs");
    ...
    const char *geometryIdStr =
        attributeList.getAttributeValue("grid.geometryId");
    const char *geometryStringStr =
        attributeList.getAttributeValue("grid.geometryString");
    ...
    if (geometryIdStr == nullptr && geometryStringStr == nullptr &&
        urnStr == nullptr && crsStr == nullptr)
      return;
    ...
    if (!def && geometryStringStr != nullptr) {
      auto defPtr =
          Identification::gridDef.createGrib2GridDefinition(geometryStringStr);
      if (!defPtr)
        return;
    ...
    if (def) {
      // If we have valid geometry definition then we know the grid width and
      // the height.

      T::Dimensions d = def->getGridDimensions();
      width = d.nx();
      height = d.ny();
      attributeList.setAttribute("grid.original.reverseYDirection",
                                 Fmi::to_string((int)def->reverseYDirection()));
      attributeList.setAttribute("grid.original.reverseXDirection",
                                 Fmi::to_string((int)def->reverseXDirection()));
      ...
      if (def->getGridMetricCellSize(wm, hm)) {
        ...
        attributeList.setAttribute("grid.original.cell.width.degrees",
                                   Fmi::to_string(wm));
        attributeList.setAttribute("grid.original.cell.height.degrees",
                                   Fmi::to_string(hm));

        def->getGridCellAverageSize(wm, hm);
        attributeList.setAttribute("grid.original.cell.width",
                                   Fmi::to_string(wm));
        attributeList.setAttribute("grid.original.cell.height",
                                   Fmi::to_string(hm));
      }
    }

    if ((urnStr != nullptr || crsStr != nullptr) &&
        (bboxStr != nullptr || llboxStr != nullptr) &&
        gridWidthStr != nullptr && gridHeightStr != nullptr) {
      // If the geometry id defined by the URN / CRS then we need to the
      // bounding box coordinates and preferred grid width and height.

      std::vector<double> aa;

      if (bboxStr != nullptr)
        splitString(bboxStr, ',', aa);
      else if (llboxStr != nullptr) {
      ...
      if (aa.size() == 4) {
        ...
        if (urnStr != nullptr) {
          ...
          if (sr.importFromURN(urn.c_str()) != OGRERR_NONE)
            throw Fmi::Exception(BCP,
                                 "Invalid urn '" + std::string(urnStr) + "'!");
        } else if (proj4Str != nullptr) {
          OGRErr err = sr.SetFromUserInput(proj4Str);
          if (err != OGRERR_NONE)
            throw Fmi::Exception(BCP, "Invalid proj4 '" +
                                          std::string(proj4Str) + "'!");
        } else if (crsStr != nullptr) {
          OGRErr err = sr.SetFromUserInput(crsStr);
          if (err != OGRERR_NONE)
            throw Fmi::Exception(BCP,
                                 "Invalid crs '" + std::string(crsStr) + "'!");
        }
        ...
        if (bboxStr != nullptr)
          ...
          reverseTransformation =
              make_ptr(OGRCreateCoordinateTransformation(&sr_latlon, &sr));
          if (not reverseTransformation) {
            throw Fmi::Exception(BCP,
                                 "Cannot create coordinate transformation!");
          }
        ...
        if (!latLonCoordinates || latLonCoordinates->size() == 0) {
          ...
          for (int t = 0; t < c; t++) {
            latLonCoordinates->emplace_back(
                T::Coordinate(getLongitude(lon[t]), lat[t]));
          }
          ...
          mCoordinateCache.insert(
              std::pair<std::size_t, CoordinateRec>(hash, rec));
        }
      }
    }
  } catch (...) {
...
void GridDef::getGridLatLonCoordinatesByGeometry(
    T::AttributeList &attributeList, T::Coordinate_svec &latLonCoordinates,
    uint &width, uint &height) {
  FUNCTION_TRACE
  try {
    ...
    const char *urnStr = attributeList.getAttributeValue("grid.urn");
    const char *bboxStr = attributeList.getAttributeValue("grid.bbox");
    const char *crop_bboxStr =
        attributeList.getAttributeValue("grid.crop.bbox");
    const char *llboxStr = attributeList.getAttributeValue("grid.llbox");
    const char *crop_llboxStr =
        attributeList.getAttributeValue("grid.crop.llbox");
    const char *geometryIdStr =
        attributeList.getAttributeValue("grid.geometryId");
    const char *geometryStringStr =
        attributeList.getAttributeValue("grid.geometryString");
    const char *gridWidthStr = attributeList.getAttributeValue("grid.width");
    const char *gridHeightStr = attributeList.getAttributeValue("grid.height");
    const char *gridCellWidthStr =
        attributeList.getAttributeValue("grid.original.cell.width");
    const char *gridCellHeightStr =
        attributeList.getAttributeValue("grid.original.cell.height");
    const char *gridCellWidthDegrStr =
        attributeList.getAttributeValue("grid.original.cell.width.degrees");
    const char *gridCellHeightDegrStr =
        attributeList.getAttributeValue("grid.original.cell.height.degrees");
    const char *gridOriginalProjectionStr =
        attributeList.getAttributeValue("grid.original.projectionType");
    ...
    if ((urnStr != nullptr || crsStr != nullptr) &&
        (bboxStr != nullptr || llboxStr != nullptr) &&
        ((gridWidthStr != nullptr && gridHeightStr != nullptr) ||
         (gridCellWidthStr != nullptr && gridCellHeightStr != nullptr))) {
      // If the geometry id defined by the URN then we need to the bounding box
      // coordinates and preferred grid width and height.
      ...
      if (aa.size() == 4) {
        ...
        if (gridWidthStr != nullptr && gridHeightStr != nullptr) {
          ...
          if (gridCellWidthStr != nullptr && gridCellHeightStr != nullptr) {
            ...
            // Size of the original grid cell (this might be metric or in
            // degrees)
            ...
            if (targetIsLatlon) {
              ...
              if (origProjectionType == T::GridProjectionValue::LatLon ||
                  origProjectionType == T::GridProjectionValue::RotatedLatLon) {
                // The source projection is also latlon.
                if (gridCellWidthDegrStr != nullptr &&
                    gridCellHeightDegrStr != nullptr) {
        ...
        if (targetIsLatlon) {
          attributeList.setAttribute("grid.cell.width",
                                     Fmi::to_string(fabs(dx)));
          attributeList.setAttribute("grid.cell.height",
                                     Fmi::to_string(fabs(dy)));
        } else {
          attributeList.setAttribute("grid.cell.width",
                                     Fmi::to_string(fabs(dx / 1000)));
          attributeList.setAttribute("grid.cell.height",
                                     Fmi::to_string(fabs(dy / 1000)));
        }
    ...
    if (latLonCoordinates && latLonCoordinates->size() > 0) {
      ...
      char tmp[100];
      sprintf(tmp, "%f,%f,%f,%f", (*latLonCoordinates)[0].x(),
              (*latLonCoordinates)[0].y(), (*latLonCoordinates)[last].x(),
              (*latLonCoordinates)[last].y());
...
GRIB1::GridDef_sptr
GridDef::getGrib1DefinitionByGeometryString(const char *geometryStr) {
  FUNCTION_TRACE
  try {
    auto it = mGridDefinitions1.find(geometryStr);
    if (it != mGridDefinitions1.end())
...
GRIB1::GridDef_sptr
GridDef::getGrib1DefinitionByGeometryString(std::string &geometryStr) {
  FUNCTION_TRACE
  try {
    auto it = mGridDefinitions1.find(geometryStr);
    if (it != mGridDefinitions1.end())
...
GRIB2::GridDef_sptr
GridDef::getGrib2DefinitionByGeometryString(const char *geometryStr) {
  FUNCTION_TRACE
  try {
...
GRIB2::GridDef_sptr
GridDef::getGrib2DefinitionByGeometryString(std::string &geometryStr) {
  FUNCTION_TRACE
  try {
    auto it = mGridDefinitions2.find(geometryStr);
    if (it != mGridDefinitions2.end())
...
bool GridDef::getGridDirectionsByGeometryId(T::GeometryId geometryId,
...
T::Coordinate_svec GridDef::getGridLatLonCoordinateLinePointsByGeometryId(
...
bool GridDef::getGridDimensionsByGeometryId(T::GeometryId geometryId,
                                            uint &cols, uint &rows) {
  FUNCTION_TRACE
  try {
    updateCheck();
...
bool GridDef::getGridPointByGeometryIdAndLatLonCoordinates(
    T::GeometryId geometryId, double lat, double lon, double &grid_i,
    double &grid_j) {
  FUNCTION_TRACE
  try {
    updateCheck();
...
bool GridDef::getGridLatLonCoordinatesByGeometryId(
    T::GeometryId geometryId, T::Coordinate_svec &coordinates) {
  FUNCTION_TRACE
  try {
    updateCheck();
    AutoReadLock lock(&mModificationLock);
...
void GridDef::getGeometryIdListByLatLon(
    double lat, double lon, std::set<T::GeometryId> &geometryIdList) {
  FUNCTION_TRACE
  try {
    updateCheck();
    AutoReadLock lock(&mModificationLock);
    ...
    for (auto it = mGridDefinitions2.begin(); it != mGridDefinitions2.end();
      ...
      if (it->second->getGridPointByLatLonCoordinates(lat, lon, grid_i,
                                                      grid_j)) {
        T::GeometryId geometryId = it->second->getGridGeometryId();
        geometryIdList.insert(geometryId);
      } else {
      ...
      if (fgets(st, 1000, file) != nullptr && st[0] != '#') {
        ...
        if (def1 != nullptr) {
          ...
          if (mGridDefinitions1.find(gstr) == mGridDefinitions1.end())
            mGridDefinitions1.insert(
                std::pair<std::string, GRIB1::GridDefinition_sptr>(
                    gstr, GRIB1::GridDefinition_sptr(def1)));
          else
            mGridDefinitions1.insert(
                std::pair<std::string, GRIB1::GridDefinition_sptr>(
                    gstr + Fmi::to_string(cnt),
                    GRIB1::GridDefinition_sptr(def1)));
        }
        ...
        if (def2 != nullptr) {
          ...
          if (mGridDefinitions2.find(gstr) == mGridDefinitions2.end())
            mGridDefinitions2.insert(
                std::pair<std::string, GRIB2::GridDefinition_sptr>(
                    gstr, GRIB2::GridDefinition_sptr(def2)));
          else
            mGridDefinitions2.insert(
                std::pair<std::string, GRIB2::GridDefinition_sptr>(
                    gstr + Fmi::to_string(cnt),
                    GRIB2::GridDefinition_sptr(def2)));
        }
      }
    }
    fclose(file);
  ...
  try {
    ...
    if (c > 1) {
      ...
      case T::GridProjectionValue::LatLon: {
        ...
        else if (strcasecmp(scanningMode, "-x+y") == 0)
        ...
        mGeometryNames.insert(
            std::pair<uint, std::string>(geometryId, geometryName));
      ...
      case T::GridProjectionValue::PolarStereographic: {
        ...
        GRIB1::PolarStereographicImpl *def1 =
            new GRIB1::PolarStereographicImpl();
        GRIB1::ScanningModeSettings scanningMode1;
  ...
  try {
    ...
    if (c > 1) {
      ...
      case T::GridProjectionValue::RotatedLatLon: {
        ...
        int jInc = C_INT(round(toDouble(field[8]) * 1000000));
        char *scanningMode = field[9];
        int longitudeOfSouthernPole =
            C_INT(round(toDouble(field[10]) * 1000000));
        int latitudeOfSouthernPole =
            C_INT(round(toDouble(field[11]) * 1000000));
        int lastLongitude = longitude + ni * iInc - iInc;
        int lastLatitude = latitude + nj * jInc - jInc;
        int angle = round(toDouble(field[12]));
        double earthSemiMajor = toDouble(field[13]);
        ...
        rotation2.setLongitudeOfSouthernPole(
            T::UInt32_opt(longitudeOfSouthernPole));

        rotation2.setLatitudeOfSouthernPole(
            T::Int32_opt(latitudeOfSouthernPole));
      ...
      case T::GridProjectionValue::PolarStereographic: {
        ...
        GRIB2::PolarStereographicImpl *def2 =
            new GRIB2::PolarStereographicImpl();
        GRIB2::ScanningModeSettings scanningMode2;
      ...
      case T::GridProjectionValue::LambertConformal: {
        ...
        int longitudeOfSouthernPole =
            C_INT(round(toDouble(field[13]) * 1000000));
        int latitudeOfSouthernPole =
            C_INT(round(toDouble(field[14]) * 1000000));
        int laD = 60000000;
        if (c >= 15)
        ...
        def2->setLongitudeOfSouthernPole(
            T::UInt32_opt(longitudeOfSouthernPole));
      ...
      case T::GridProjectionValue::LambertAzimuthalEqualArea: {
        ...
        GRIB2::LambertAzimuthalEqualAreaImpl *def2 =
            new GRIB2::LambertAzimuthalEqualAreaImpl();
        GRIB2::ScanningModeSettings scanningMode2;
...
FmiParamId_grib_cptr
GridDef::getGribParameterMappingByFmiId(T::FmiParamId fmiParamId) {
  FUNCTION_TRACE
  try {
...
T::GribParamId GridDef::getGribParameterId(GRIB1::Message &message) {
  ...
  try {
    ...
    for (auto p = mGrib1_parameterDef_cache.begin();
         p != mGrib1_parameterDef_cache.end(); ++p) {
      uint requiredPoints = p->mParameterList.size();
      uint points = 0;
      bool failed = false;

      for (auto pm = p->mParameterList.begin();
           pm != p->mParameterList.end() && !failed; ++pm) {
        if (message.hasAttributeValue(stringFactory[pm->first],
                                      pm->second.c_str()))
          points++;
        else
          failed = true;
    ...
    for (auto p = mGrib1_parameterDef_records.begin();
         p != mGrib1_parameterDef_records.end(); ++p) {
      uint requiredPoints = p->mParameterList.size();
...
T::GribParamId GridDef::getGribParameterId(GRIB2::Message &message) {
  ...
  try {
    uint maxPoints = 0;
    T::GribParamId parameterId = 0;
    for (auto p = mGrib2_parameterDef_cache.begin();
         p != mGrib2_parameterDef_cache.end(); ++p) {
      uint requiredPoints = p->mParameterList.size();
      uint points = 0;
      bool failed = false;
    ...
    for (auto p = mGrib2_parameterDef_records.begin();
         p != mGrib2_parameterDef_records.end(); ++p) {
      uint requiredPoints = p->mParameterList.size();
      if (maxPoints < requiredPoints) {
        uint points = 0;
        bool failed = false;
...
bool GridDef::getGrib1LevelDef(uint fmiLevelId, FmiLevelId_grib &def) {
  FUNCTION_TRACE
  try {
    for (auto it = mFmi_levelsFromGrib1_records.begin();
         it != mFmi_levelsFromGrib1_records.end(); ++it) {
...
bool GridDef::getGrib2LevelDef(uint fmiLevelId, FmiLevelId_grib &def) {
  FUNCTION_TRACE
  try {
    for (auto it = mFmi_levelsFromGrib2_records.begin();
         it != mFmi_levelsFromGrib2_records.end(); ++it) {
      if (it->mFmiLevelId == fmiLevelId) {
        def = *it;
        return true;
      }
...
T::ParamLevelId GridDef::getFmiLevelId(GRIB1::Message &message) {
  ...
  try {
    ...
    for (auto it = mFmi_levelsFromGrib1_records.begin();
      ...
      int matchCount = 0;
      for (auto itm = it->mParameterList.begin();
           itm != it->mParameterList.end(); ++itm) {
        if (message.hasAttributeValue(stringFactory[itm->first],
                                      itm->second.c_str()))
          matchCount++;
      }
...
T::FmiParamId
GridDef::getFmiParameterIdByFmiName(const std::string &fmiParamName) {
  FUNCTION_TRACE
  try {
    updateCheck();
    AutoReadLock lock(&mModificationLock);
...
T::FmiParamId
GridDef::getFmiParameterIdByNewbaseName(const std::string &newbaseParamName) {
  FUNCTION_TRACE
  try {
    updateCheck();
    AutoReadLock lock(&mModificationLock);
...
T::FmiParamId
GridDef::getFmiParameterIdByNetCdfName(const std::string &netCdfParamName) {
  FUNCTION_TRACE
  try {
    updateCheck();
    AutoReadLock lock(&mModificationLock);
...
T::FmiParamId
GridDef::getFmiParameterIdByNewbaseId(T::NewbaseParamId newbaseParamId) {
  FUNCTION_TRACE
  try {
    updateCheck();
    AutoReadLock lock(&mModificationLock);
...
FmiParamId_newbase_cptr
GridDef::getFmiParameterMappingByNewbaseId(T::NewbaseParamId newbaseParamId) {
  FUNCTION_TRACE
  try {
...
FmiParamId_netCdf_cptr
GridDef::getFmiParameterMappingByNetCdfName(std::string &netCdfParamName) {
  FUNCTION_TRACE
  try {
    std::string key = toUpperString(netCdfParamName);
...
FmiParamId_grib_cptr
GridDef::getFmiParameterMappingByGribId(T::GribParamId gribParamId) {
  FUNCTION_TRACE
  try {
...
FmiParamId_newbase_cptr
GridDef::getNewbaseParameterMappingByFmiId(T::FmiParamId fmiParamId) {
  FUNCTION_TRACE
  try {
...
FmiParamId_netCdf_cptr
GridDef::getNetCdfParameterMappingByFmiId(T::FmiParamId fmiParamId) {
  FUNCTION_TRACE
  try {
...
bool GridDef::getNewbaseParameterDefByFmiId(T::FmiParamId fmiParamId,
                                            NewbaseParameterDef &paramDef) {
  FUNCTION_TRACE
  try {
...
bool GridDef::getNetCdfParameterDefByFmiId(T::FmiParamId fmiParamId,
                                           NetCdfParameterDef &paramDef) {
  FUNCTION_TRACE
  try {
...
bool GridDef::getNewbaseParameterDefByName(const std::string &newbaseParamName,
                                           NewbaseParameterDef &paramDef) {
  FUNCTION_TRACE
  try {
...
bool GridDef::getNetCdfParameterDefByName(const std::string &netCdfParamName,
                                          NetCdfParameterDef &paramDef) {
  FUNCTION_TRACE
  try {
...
bool GridDef::getNewbaseParameterMappingByFmiId(
    T::FmiParamId fmiParamId, FmiParameterId_newbase &paramMapping) {
  try {
...
bool GridDef::getNetCdfParameterMappingByFmiId(
    T::FmiParamId fmiParamId, FmiParameterId_netCdf &paramMapping) {
  try {
...
bool GridDef::getNewbaseParameterDefById(T::NewbaseParamId newbaseParamId,
                                         NewbaseParameterDef &paramDef) {
  FUNCTION_TRACE
  try {
...
FmiParamDef_cptr
GridDef::getFmiParameterDefByNewbaseId(T::NewbaseParamId newbaseParamId) {
  FUNCTION_TRACE
  try {
...
FmiParamDef_cptr
GridDef::getFmiParameterDefByNetCdfName(std::string &netCdfParamName) {
  FUNCTION_TRACE
  try {
...
FmiParamDef_cptr
GridDef::getFmiParameterDefByGribId(T::GribParamId gribParamId) {
  FUNCTION_TRACE
  try {
...
FmiParamDef_cptr
GridDef::getFmiParameterDefByName(const std::string &fmiParamName) {
  FUNCTION_TRACE
  try {
...
bool GridDef::getFmiParameterDefById(T::FmiParamId fmiParamId,
                                     FmiParameterDef &paramDef) {
  FUNCTION_TRACE
  try {
    updateCheck();
    AutoReadLock lock(&mModificationLock);
...
bool GridDef::getFmiParameterDefByNewbaseId(T::NewbaseParamId newbaseParamId,
                                            FmiParameterDef &paramDef) {
  FUNCTION_TRACE
  try {
    updateCheck();
...
bool GridDef::getFmiParameterDefByNetCdfName(std::string &netCdfParamName,
                                             FmiParameterDef &paramDef) {
  FUNCTION_TRACE
  try {
    updateCheck();
...
bool GridDef::getFmiParameterDefByGribId(T::GribParamId gribParamId,
                                         FmiParameterDef &paramDef) {
  FUNCTION_TRACE
  try {
    updateCheck();
...
bool GridDef::getFmiParameterDefByName(const std::string &fmiParamName,
                                       FmiParameterDef &paramDef) {
  FUNCTION_TRACE
  try {
    updateCheck();
...
NewbaseParamDef_cptr
GridDef::getNewbaseParameterDefById(T::NewbaseParamId newbaseParamId) {
  FUNCTION_TRACE
  try {
    if (newbaseParamId == 0)
      return nullptr;
...
NewbaseParamDef_cptr
GridDef::getNewbaseParameterDefByName(const std::string &newbaseParamName) {
  FUNCTION_TRACE
  try {
    std::string key = toUpperString(newbaseParamName);
...
NetCdfParamDef_cptr
GridDef::getNetCdfParameterDefByName(const std::string &netCdfParamName) {
  FUNCTION_TRACE
  try {
    std::string key = toUpperString(netCdfParamName);
